...
Usage:
First download preprocessed dataset and pre-trained model files from the links below in the original README.
```
cd 3DGAN/
```
To run quantitative tests (PSNR & SSIM scores) on pre-trained model (MSE reconstruction loss):
```
python test.py --ymlpath=./experiment/multiview2500/d2_multiview2500.yml --gpu=0 --dataroot=./data/LIDC-HDF5-256 --dataset=test1 --tag=d2_multiview2500 --data=LIDC256 --dataset_class=align_ct_xray_views_std --model_class=MultiViewCTGAN --datasetfile=./data/test.txt --resultdir=./multiview --check_point=90 --how_many=3
```
To generate visualization of CT volumes of the pre-trained model (MSE reconstruction loss):
```
python visual.py --ymlpath=./experiment/multiview2500/d2_multiview2500.yml --gpu=0 --dataroot=./data/LIDC-HDF5-256 --dataset=test1 --tag=d2_multiview2500 --data=LIDC256 --dataset_class=align_ct_xray_views_std --model_class=MultiViewCTGAN --datasetfile=./data/test.txt --resultdir=./multiview --check_point=90 --how_many=3
```
To train a new model using 3D perceptual loss:
```
python train.py --ymlpath=./experiment/multiview2500/d2_multiview2500_pl.yml --gpu=0,1,2,3 --dataroot=./data/LIDC-HDF5-256 --dataset=train --tag=d2_multiview2500pl --data=LIDC256_perceptualloss --dataset_class=align_ct_xray_views_std --model_class=MultiViewCTGAN --datasetfile=./data/train.txt --valid_datasetfile=./data/test.txt --valid_dataset=test --check_point=180
```
To run quantitative tests (PSNR & SSIM scores) on the newly trained model (3D perceptual loss):
```
python test.py --ymlpath=./experiment/multiview2500/d2_multiview2500_pl.yml --gpu=0 --dataroot=./data/LIDC-HDF5-256 --dataset=test --tag=d2_multiview2500pl --data=LIDC256_perceptualloss --dataset_class=align_ct_xray_views_std --model_class=MultiViewCTGAN --datasetfile=./data/test.txt --resultdir=./multiview --check_point=90 --how_many=3
```
To generate visualization of CT volumes of the newly trained model (3D perceptual loss):
```
python visual.py --ymlpath=./experiment/multiview2500/d2_multiview2500_pl.yml --gpu=0 --dataroot=./data/LIDC-HDF5-256 --dataset=test --tag=d2_multiview2500pl --data=LIDC256_perceptualloss --dataset_class=align_ct_xray_views_std --model_class=MultiViewCTGAN --datasetfile=./data/test.txt --resultdir=./multiview --check_point=90 --how_many=3
